...
#   scripts/deploy/README-DEPLOY-VPS.md
#
# Uso:
#   DEPLOY_I_READ_README=yes ./scripts/deploy-vps.sh
#   (canónico: ./scripts/deploy/deploy-vps.sh)
set -euo pipefail
...
# Bind-mounted entrypoint lives under server/YurOTS/; keep it in sync with scripts/deploy/.
if [[ -f "$ROOT/scripts/deploy/docker-entrypoint.sh" ]]; then
  cp -a "$ROOT/scripts/deploy/docker-entrypoint.sh" "$ROOT/server/YurOTS/docker-entrypoint.sh"
