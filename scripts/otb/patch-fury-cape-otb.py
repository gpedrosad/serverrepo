#!/usr/bin/env python3
"""Patch server id 20114 from ironbite axe (weapon) to fury cape (armor)."""
...
ITEM_GROUP_ARMOR = 5
TARGET_SERVER_ID = 20114
TARGET_CLIENT_ID = 4840
PROTOTYPE_SERVER_ID = 2654
ITEM_NAME = "fury cape"
ITEM_DESCR = (
    "A blazing cape torn from the heart of a fury. "
    "Sorcerers and druids gain +1 magic level while wearing it."
)
...
def patch_otb(path: Path) -> bool:
    ...
    blocks = [block for block in blocks if block[0] not in (ITEM_ATTR_SERVERID, ITEM_ATTR_CLIENTID, ITEM_ATTR_NAME, ITEM_ATTR_DESCR)]
