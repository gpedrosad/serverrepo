...
public:
  ...
  enum_world_type getWorldType() const {return worldType;}
	...
	bool trySpawnRageMonster(const Monster* slainMonster, const Position& deathPos);
  ...
  void FreeThing(Thing* thing);
	  ...
	  ReturnValue canThrowObjectTo(const Position& from, const Position& to, int objectstate = BLOCK_PROJECTILE);
  ...
  OTSYS_THREAD_LOCKVAR gameLock;
	...
	void serverSave();
	void autoServerSave();
	// Delayed save after red warning to all online players (midnight cron).
	void scheduledServerSave();
	// Poll data/.request-server-save (5 min warn) / .request-server-save-now
	// and SIGTERM/SIGINT flags so deploy can flush players before restart.
