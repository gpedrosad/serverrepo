...
bool Game::creatureMakeMagic(Creature *creature, const Position& centerpos, const MagicEffectClass* me)
{
	...
	if(targettile && me->canCast(isBlocking, hasTarget)) {
		bSuccess = true;
		...
		if(tmp){
			...
			if( sit != tmp->end() ) {
				if(player->getEffectiveMagLevel() >= sit->second->getMagLv()){
					...
					std::stringstream msg;
					msg << "You need magic level " << sit->second->getMagLv() << " to cast this spell.";
				...
				if(ait != spells.getAllSpells()->end()) {
					...
					std::string needed;
					for(int v = 1; v <= 4; v++) {
						std::map<std::string, Spell*>* vm = spells.getVocSpells((playervoc_t)v);
						if(vm && vm->find(temp) != vm->end()) {
					...
					std::stringstream msg;
					if(needed.empty())
						msg << "You cannot cast this spell.";
					else
						msg << "You need to be a " << needed << " to cast this spell.";
					player->sendTextMessage(MSG_RED_INFO, msg.str().c_str());
				}
			}
		}
